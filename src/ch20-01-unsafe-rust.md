...
静态（`static`）变量类似于第三章 [“常量”][constants] 部分讨论的常量。通常静态变量的名称采用 `SCREAMING_SNAKE_CASE` 写法。静态变量只能储存拥有 `'static` 生命周期的引用，这意味着 Rust 编译器可以自己计算出其生命周期而无需显式标注。访问不可变静态变量是安全的。
...
作为一个例子，回忆第十六章 [“使用 `Sync` 和 `Send` trait 的可扩展并发”][send-and-sync] 部分中的 `Sync` 和 `Send` 标记 trait：如果我们的类型完全由实现了 `Send` 与 `Sync` 的其他类型组成，编译器会自动为其实现这些 trait。如果我们定义的类型包含某些未实现 `Send` 或 `Sync` 的类型，例如裸指针，但又想将该类型标记为 `Send` 或 `Sync`，就必须使用 `unsafe`。Rust 不能验证我们的类型保证可以安全地跨线程发送或在多线程间访问，所以需要我们自己进行检查，并通过 `unsafe` 表明这一点。
...
Miri 正确地警告我们：我们正在把一个整数转换成指针，这可能是个问题。不过，Miri 无法判断这里是否一定有问题，因为它不知道这个指针最初是如何产生的。随后，Miri 又在我们取消引用该值的地方返回了一个错误。借助 Miri，我们现在至少知道这里存在未定义行为的风险，于是就可以进一步思考如何让这段代码变得安全。在某些情况下，Miri 甚至能够检测出真正的错误，也就是**确定有问题**的代码模式，并给出如何修复这些错误的建议。
...
换句话说：如果 Miri **捕获到了**一个问题，那你就知道这里确实有个 bug；但仅仅因为 Miri **没有**捕获某个 bug，并不意味着这里就没有问题。不过，它确实能发现很多问题。你可以试着对本章中的其他不安全代码示例运行 Miri，看看它会给出什么结果！
...
[dangling-references]: ch04-02-references-and-borrowing.html#悬垂引用

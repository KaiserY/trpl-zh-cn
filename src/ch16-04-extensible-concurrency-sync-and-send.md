...
然而，在内嵌于语言而非标准库的关键并发概念中，有属于 `std::marker` 的 `Send` 和 `Sync` trait。
...
因此，Rust 类型系统和 trait bound 确保永远也不会意外的将不安全的 `Rc<T>` 在线程间发送。当尝试在示例 16-14 中这么做的时候，会得到错误 `the trait Send is not implemented for Rc<Mutex<i32>>`。而使用实现了 `Send` 的 `Arc<T>` 时，就没有问题了。
